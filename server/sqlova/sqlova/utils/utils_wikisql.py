...
def pred_wvi_se_beam(max_wn, s_wv, beam_size):
    ...
    k_logit = min(int(ceil(sqrt(beam_size))), mL-1)
    n_pairs = k_logit**2
    # assert n_pairs >= beam_size
    # print ("MLLLLL: ", mL, k_logit)
...
def pred_sw_se_topk(s_sc, s_sa, s_wn, s_wc, s_wo, s_wv, k=5):

    results = []
    probs = []
    for pr_sc, pr_sc_prob in enumerate(F.softmax(s_sc[0])):

        if pr_sc_prob < 0.0001:
            continue

        for pr_sa, pr_sa_prob in enumerate(F.softmax(s_sa[0])):
            
            if pr_sc_prob * pr_sa_prob < 0.0001:
                continue

            for pr_wn, pr_wn_prob in enumerate(F.softmax(s_wn[0])):

                if pr_wn == 0: continue
                if pr_wn > 2 and pr_wn_prob < 0.001:
                    continue
                
                # print (pr_sc, pr_sa, pr_wn, pr_wn_prob)
                for pr_wc, pr_wc_probs in [zip(*x) for x in itertools.combinations(enumerate(F.softmax(s_wc[0])), pr_wn)]:
                    for pr_wo, pr_wo_probs in [zip(*x) for x in itertools.product(*[enumerate(x) for x in F.softmax(s_wo[0, 0:pr_wn])])]:
                        
                        vals, probs_data = pred_wvi_se_beam(pr_wn, s_wv, beam_size=100)
                        vals = np.array(vals[0]).swapaxes(0, 1)
                        probs_data = np.array(probs_data[0]).swapaxes(0, 1)

                        for pr_wvi, pr_wvi_prob in zip(vals, probs_data):
                            
                            prob = pr_sc_prob.data.item() \
                                    * pr_sa_prob.data.item() \
                                    * pr_wn_prob.data.item() \
                                    * np.product(pr_wc_probs) \
                                    * np.product(pr_wo_probs) \
                                    * np.product(pr_wvi_prob) \
                            
                            results += [([pr_sc], [pr_sa], [pr_wn], [pr_wc], [pr_wo], [pr_wvi])]
                            probs += [prob]
                            if prob > 0.001:
                                print (results[-1], prob)

    idxs = np.argsort(probs)[::-1][0:k]
    return [results[i] for i in idxs], [probs[i] for i in idxs]
...
def generate_nlp_q_base(sql_i, tb):
    sql_q = []
    for b, sql_i1 in enumerate(sql_i):
        tb1 = tb[b]
        nlp_q1 = generate_nlp_q1_base(sql_i1, tb1)
        sql_q.append(nlp_q1)
...
def generate_nlp_q1_base(sql_i1, tb1):
    """
        sql = {'sel': 5, 'agg': 4, 'conds': [[3, 0, '59']]}
        agg_ops = ['', 'max', 'min', 'count', 'sum', 'avg']
        cond_ops = ['=', '>', '<', 'OP']
        ...
        "PLUS" indicates, it deals with the some of db specific facts like PCODE <-> NAME
    """
    agg_ops = ['', 'the greatest', 'the smallest', 'the number of', 'the total amount of', 'the average']
    plural = [False, False, False, True, True, False]
    cond_ops = ['is', 'is greater than', 'is less than', 'OP']
    ...
    select_agg = agg_ops[sql_i1['agg']]
    select_header = headers[sql_i1['sel']]
    if plural[sql_i1['agg']]:
        select_header = inflect_eng.plural(select_header)

    sql_query_part1 = f'What is {select_agg} {select_header} '
    params = []
    ...
    if where_num == 0:
        ...
        sql_query_part2 = f'such that the'
        # sql_plus_query_part2 = f'FROM {select_table_refined} WHERE'
        # ----------------------------------------------------------------------------------------------------------
        for i in range(where_num):
            # check 'OR'
