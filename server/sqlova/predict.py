...
if not args.split:
    ...
    def get_columns():
        ...
        curs.execute(sql.SQL("Select * FROM {} LIMIT 0").format(sql.Identifier(request.args['table'])))
    ...
    def run_query():
        ...
        curs = connection.cursor()
        curs.execute(request.args['query'].replace("?", "%s"), [request.args['params']])
...
def predict(data_loader, data_table, model, model_bert, bert_config, tokenizer,
            max_seq_length,
            num_target_layers, detail=False, st_pos=0, cnt_tot=1, EG=False, beam_size=1,
    ...
    for iB, t in enumerate(data_loader):
        ...
        if not EG:
            ...
            print ("Result: ", pr_sc, pr_sa, pr_wn, pr_wc, pr_wo, pr_wvi)
            pr_wv_str, pr_wv_str_wp = convert_pr_wvi_to_string(pr_wvi, nlu_t, nlu_tt, tt_to_t_idx, nlu)
            pr_sql_i = generate_sql_i(pr_sc, pr_sa, pr_wn, pr_wc, pr_wo, pr_wv_str, nlu)
            pr_sql_i_list.append(pr_sql_i)

            result, probs = pred_sw_se_topk(s_sc, s_sa, s_wn, s_wc, s_wo, s_wv)
            probs = [1.0] + probs
            for i, (pr_sc, pr_sa, pr_wn, pr_wc, pr_wo, pr_wvi) in enumerate(result):
                pr_wv_str, pr_wv_str_wp = convert_pr_wvi_to_string(pr_wvi, nlu_t, nlu_tt, tt_to_t_idx, nlu)
                pr_sql_i = generate_sql_i(pr_sc, pr_sa, pr_wn, pr_wc, pr_wo, pr_wv_str, nlu)
                pr_sql_i_list.append(pr_sql_i)
                # print ("Sql of i: ", pr_sql_i, probs[i])
        ...
        pr_sql_q_list = [generate_sql_q(pr_sql_i, tb) for pr_sql_i in pr_sql_i_list]
        pr_sql_q_base_list = [generate_sql_q_base(pr_sql_i, tb) for pr_sql_i in pr_sql_i_list]
        pr_nlp_q_base_list = [generate_nlp_q_base(pr_sql_i, tb) for pr_sql_i in pr_sql_i_list]

        for pr_sql_i, pr_sql_q, pr_sql_q_base, pr_nlp_q_base, p in zip(pr_sql_i_list, pr_sql_q_list, pr_sql_q_base_list, pr_nlp_q_base_list, probs):
            for b, (pr_sql_i1, pr_sql_q1, pr_sql_q1_base, pr_nlp_q1_base) in enumerate(zip(pr_sql_i, pr_sql_q, pr_sql_q_base, pr_nlp_q_base)):
                results1 = {}
...
def handle_request0(request):
    ...
    try:
        ...
        stream = io.StringIO(csv, newline=None)
